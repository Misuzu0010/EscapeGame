...
struct FCombatActionDefinition
{
	...
	// 攻击输入缓存容忍时间 (单位：秒，表示在攻击动画的某个窗口期内，玩家输入攻击指令会被缓存并在窗口期结束时执行)
	...
	// 执行该动作时授予角色的标签 (可以用来在蓝图里查表，决定角色在执行该动作时具备哪些状态或能力)
...
struct FGeneralActionDefinition
{
	...
	// 是否可被打断 (表示在执行该动作时是否允许被其他动作打断，可以用来控制动作的优先级和连贯性)
	...
	// 耐力消耗 (单位：点，表示执行该动作时消耗的耐力值，可以用来限制动作的使用频率或持续时间)
